...
if [[ $CHOICE == 1 ]]; then
  ...
  TIMER=$(($HOURS * 60 * 60 + $MINUTES * 60 + $CURRENT_TIME))

  touch "$SCRIPT_PATH/current_timers/$TIMER"

  echo "New timer: $(date -d "@$TIMER" +%m/%d\ -\ %H:%M:%S) - $((($TIMER - $CURRENT_TIME) / 60))min remaining"
  exit
  ...
  # adjust if on the day after
  if [[ $POSSIBLE_TIMER -lt "$CURRENT_TIME" ]]; then
    POSSIBLE_TIMER=$(date -d "$(date -d "@$POSSIBLE_TIMER") + 1 day" +%s)
  fi
  echo -e "\nTIMER: $(date -d "@$POSSIBLE_TIMER" +%I:%M\ %p\ \(%H:%M\)\ \o\n\ %m/%d/%Y)"
  echo -en "(date mm/dd) \n..? "
  read -r NEW_DATE
  if [[ -n "$NEW_DATE" ]]; then
    POSSIBLE_TIMER="$(date -d "$NEW_DATE $(date -d "@$POSSIBLE_TIMER" +%H:%M)" +%s)"
    # adjust if on the year after
    if [[ "$POSSIBLE_TIMER" -lt "$CURRENT_TIME" ]]; then
      POSSIBLE_TIMER="$(date -d "$(date -d "@$POSSIBLE_TIMER") + 1 year" +%s)"
    fi
  fi
  echo -e "\nTIMER: $(date -d "@$POSSIBLE_TIMER" +%I:%M\ %p\ \(%H:%M\)\ \o\n\ %m/%d/%Y)"
  echo -en "Press enter to confirm \n..? "
  read -r CHOICE

  if [[ -z $CHOICE ]]; then
    touch "$SCRIPT_PATH/current_timers/$POSSIBLE_TIMER"
    echo "New timer: $(date -d "@$POSSIBLE_TIMER" +%m/%d\ -\ %H:%M:%S) - $((($POSSIBLE_TIMER - $CURRENT_TIME) / 60))min remaining"
  else
    echo "Alarm initiation canceled." >&2
  fi
  ...
  if [[ -n $TIMERS ]]; then
    ...
    i=1
    while [[ -n $TEMP ]]; do
      echo "($i)  $(date -d "@$TEMP" +%m/%d\ -\ %H:%M:%S) - $((($TEMP - $CURRENT_TIME) / 60))min remaining"
      i=$(($i+1))
